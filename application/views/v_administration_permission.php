...
<div class="">
    ...
    <tbody>
      	<?php  $i=1; foreach($func as $details){?>
        <tr>
       
          <td style="max-width: 210px;"><?php echo $details->function;?></td>
          <td> <?php 
          		if($details->normal==0){
          				echo '<input name="chk'.$i.'" value="'.$i.'" id="checknor" type="checkbox">';
					}else{
						echo '<input name="chk'.$i.'" value="'.$i.'" id="checknor" type="checkbox" checked="yes">';
					}    $i++;      	
          	?> 
			  </td>
          <td> <?php 
          		if($details->business==0){
          				echo '<input name="chk'.$i.'" value="'.$i.'" id="checkbusi" type="checkbox">';
					}else{
						echo '<input name="chk'.$i.'" value="'.$i.'" id="checkbusi" type="checkbox" checked="yes">';
					}          	$i++;
          	?>  
			  </td>
			<td> <?php 
          		if($details->admin==0){
          				echo '<input name="chk'.$i.'" value="'.$i.'" id="checkadmin" type="checkbox">';
					}else{
						echo '<input name="chk'.$i.'" value="'.$i.'" id="checkadmin" type="checkbox" checked="yes">';
					}          	$i++;
          	?> 
			  </td>
			<td> <?php 
          		if($details->unregistered==0){
          				echo '<input name="chk'.$i.'" value="'.$i.'" id="checkunreg" type="checkbox">';
					}else{
						echo '<input name="chk'.$i.'" value="'.$i.'" id="checkunreg" type="checkbox" checked="yes">';
					}          	
          	?> 
			  </td>
